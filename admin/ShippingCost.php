...
<head>
    ...
    <style>
        ...
        #addshipping_form select {
            padding: 5px;
            width: 80%;
        }

        #addshipping_form label {
            font-size: 17px;
        }

        #addshipping_form input {
...
<body>
            ...
            <div class="modal-content">
                ...
                <div class="modal-body">
                    <form id="s_m_form" method="post">
                        ...
                        <br>
                        <input type="number" placeholder="Amount" id="country_shipping_cost">
                        <input type="number" hidden placeholder="shipping id " id="shipping_id">
    ...
    <!-- Modal for  eidt   shipping cost    -->



    <!-- Modal for  delete shipping  -->
    <div class="modal fade" id="deletemodal" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel" aria-hidden="true">
        <div class="modal-dialog" role="document">
            <div class="modal-content">
                <div class="modal-header">
                    <h5 class="modal-title" id="exampleModalLabel">Delete Shipping Cost </h5>
                    <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                        <span aria-hidden="true">&times;</span>
                    </button>
                </div>
                <div class="modal-body">
                    <p style="color:black">Are you sure you want to delete this Shipping cost ?!</p>
                </div>
                <div class="modal-footer">
                    <button type="button" class="btn btn-secondary" data-dismiss="modal">Close</button>
                ...
                <div class="modal-body">
                    ...
                    <form id="addshipping_form" method="post">
                        <select class="form-select" aria-label="Default select example">
                            ...
                            <?php
                            $query  = $mysqli->query("SELECT * FROM tbl_country");
                        ...
                        <input type="number" placeholder="Amount for Shipping  ">
    ...
    <div class="search">
        <form method="post">
            <input name="search" type="text" placeholder="Search for Sizes">
    ...
    <div class="container">
        ...
        <table>
            ...
            <?php
            if (isset($_POST["all"])) {
                $q  = $mysqli->query("SELECT * FROM  tbl_shipping_cost ");
                $counter  = 1;
                if (mysqli_num_rows($q) ==  0) {
                    ...
                    while ($row1 = mysqli_fetch_assoc($q)) {
                        ...
                        $queryforcountryname  = $mysqli->query("SELECT  * FROM  tbl_country WHERE country_id = '$countryid'");
                        while ($row = mysqli_fetch_assoc($queryforcountryname)) {
                            $countryanme  = $row["country_name"];
                        }
                        ...
                        echo  '<input hidden   type="text"  name ="country_id" value= "' . $countryid . '">';
                        echo  '<input  hidden  type="text"  name ="country_name" value= "' . $row1["country_name"] . '">';
                        echo  '<input  hidden  type="text"  name ="country_name" value= "' . $row1["amount"] . '">';
            ...
            if ($_POST["pagination"]) {
                ...
                $querypageination  = $mysqli->query("SELECT * FROM  tbl_shipping_cost  LIMIT $start,5 ");
                $counter  = $start + 1;
                if (mysqli_num_rows($querypageination) ==  0) {
                    ...
                    while ($row1 = mysqli_fetch_assoc($querypageination)) {
                        $countryid = $row1["country_id"];
                        $queryforcountryname  = $mysqli->query("SELECT  * FROM  tbl_country WHERE country_id = '$countryid'");
                        while ($row = mysqli_fetch_assoc($queryforcountryname)) {
                            $countryanme  = $row["country_name"];
                        ...
                        echo "<td class='action'>";
                        echo "<form method='post' id='edittcat'>";
                        echo  '<input  hidden  type="text"  name ="country_id" value= "' . $row["country_id"] . '">';
                        echo  '<input  hidden  type="text"  name ="country_name" value= "' . $row1["country_name"] . '">';
                        echo  '<input  hidden  type="text"  name ="country_name" value= "' . $row1["amount"] . '">';
            ...
            if (isset($_POST["searchstr"])) {
                ...
                $query1  = $mysqli->query("SELECT * FROM  tbl_shipping_cost WHERE country_name LIKE '$str%' ");
                ...
                if (mysqli_num_rows($query1) ==  0) {
                    ...
                    while ($row1 = mysqli_fetch_assoc($query1)) {
                        ...
                        echo "<td class='action'>";
                        echo "<form method='post' id='edittcat'>";
                        echo  '<input  hidden  type="text"  name ="size_id" value= "' . $row1["country_id"] . '">';
                        echo  '<input  hidden  type="text"  name ="country_name" value= "' . $row1["country_name"] . '">';
                        echo  '<input  hidden  type="text"  name ="country_name" value= "' . $row1["amount"] . '">';
            ...
            <i class='bx bx-message-square-minus'></i>
                ...
                if ($_POST["pagination"] != "true"  && !isset($_POST["all"])) {
                    $query  = $mysqli->query("SELECT * FROM  tbl_shipping_cost limit 5");
                    $counter  = 1;
        ...
        <div class="pagination  mt-4 ">
            <?php
            $query2  = $mysqli->query("SELECT * FROM  tbl_shipping_cost ");
    ...
    <script>
        $("#edittcat button").click(function() {
            ...
            var shipping_id = s[0].value;
            var countryname = s[1].value;
            var shippingcost = s[2].value;

            $(".country_name").html(countryname);
            $("#country_shipping_cost").val(shippingcost);
            $("#shipping_id").val(shipping_id);
        })


        $("#eidtsocial_media").click(function() {
            var newshippingcost = $("#country_shipping_cost").val();
            var shipping_id_toedit = $("#shipping_id").val()
            console.log(shipping_id_toedit) ;

            $.ajax({
                type: "post",
                url: "edit_add_delete_shipping.php?edit=ture",
                data: {
                    newshippingcost:newshippingcost,
                    shipping_id_toedit:shipping_id_toedit
                },
        ...
        $(".de").click(function() {
            ...
            $("#delete_size").click(function() {
                setTimeout(function() {
                    $.ajax({
                        type: "post",
                        url: "edit_add_delete_shipping.php?delete=true",
                        data: {
        ...
        $("#addsocilamedia").click(function() {
            ...
            var Country_id = $("#addshipping_form select ").val();
            var Amount = $("#addshipping_form input ").val();

            if (Amount == "" || Country_id == "Select the Country") {
                $(".message").css("display", "block");
                $(".message").html(" Do not leave fields  empty!");
                setInterval(() => $(".message").fadeOut(), 5000)
            } else {
                $.ajax({
                    type: "post",
                    url: "edit_add_delete_shipping.php?add=true",
                    data: {
                        Amount:Amount,
                        Country_id: Country_id
                    },
